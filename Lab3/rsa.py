...
def make_key_pair(length):
    ...
    if length < 4:
        raise ValueError(
            "cannot generate a key of length less "
    ...
    while primes:
        ...
        raise AssertionError(
            "cannot find 'p' and 'q' for a key of " "length={!r}".format(length)
        )
    ...
    for e in range(3, stop, 2):
        ...
        raise AssertionError(
            "cannot find 'e' with p={!r} " "and q={!r}".format(p, q)
        )
    ...
    for d in range(3, stop, 2):
        ...
        raise AssertionError(
            "cannot find 'd' with p={!r}, q={!r} " "and e={!r}".format(p, q, e)
        )
